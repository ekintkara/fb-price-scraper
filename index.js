    ...
    for (const playerEndpoint of players) {
      try {
        ...
        const session = await fetch(
          process.env.CF_SCRAPER_ENDPOINT,
          {
            method: "POST",
            headers: {
        ...
        if (!session || session.code != 200) {
          console.error("Session alınamadı:", session);
          continue;
        }
        ...
        const playerCards = {};
        const cardCount = $('.player-header-card-section.full-height.minus-margin-top-16 .player-card-wrapper').length;
        console.log(`\nBu futbolcu için ${cardCount} farklı kart bulundu.`);

        $('.player-header-card-section.full-height.minus-margin-top-16 .player-card-wrapper').each((_, cardWrapper) => {
          ...
          if ($playerImg.length > 0) {
            ...
            if (playerIdMatch && playerIdMatch[1]) {
              ...
              console.log(`Kart bulundu - DataID: ${dataId}, PlayerExternalId: ${playerExternalId}`);
            } else {
              console.log(`Uyarı: ${imgSrc} URL'sinden playerExternalId çıkarılamadı`);
            }
          } else {
        ...
        for (const [dataId, playerExternalId] of Object.entries(playerCards)) {
          ...
          $(`.player-header-prices-section [data-id="${dataId}"]`).each((_, priceBlock) => {
            ...
            const $firstPrice = $block.find("div.column > div.price.inline-with-icon.lowest-price-1");
            if ($firstPrice.length > 0) {
              const firstPriceText = $firstPrice.clone().children().remove().end().text().trim();
              const firstPriceValue = parseInt(firstPriceText.replace(/,/g, ""));
            ...
            const $otherPrices = $block.find(".lowest-prices-wrapper .lowest-price.inline-with-icon");
            $otherPrices.each((index, element) => {
              const $price = $(element);
              const priceText = $price.clone().children().remove().end().text().trim();
              const priceValue = parseInt(priceText.replace(/,/g, ""));
              if (!isNaN(priceValue)) {
                priceData[`${prefix}Price${index + 2}`] = priceValue;
                console.log(`${prefix} için ${index + 2}. fiyat:`, priceValue);
              }
            });
          
            const $lastUpdated = $block.find(".prices-updated.no-wrap.inline-with-icon.text-faded");
            if ($lastUpdated.length > 0) {
              ...
              if (timeMatch && timeMatch[1]) {
                ...
                console.log(`${prefix} için son güncelleme:`, timeMatch[1].trim());
              }
            }
          });
        ...
        if (results.Data.length > 0) {
          try {
            ...
            console.log(`Veriler başarıyla gönderildi. API yanıtı: ${apiResponse.status}`);
          } catch (apiError) {
        ...
        await new Promise((resolve) => setTimeout(resolve, parseInt(process.env.RATE_LIMIT_MS)));
      } catch (playerError) {
